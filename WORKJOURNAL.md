...
- **Ignore excluded + logseq/ folders** — both adapters now skip pages outside user content when building the index. New pure `isInLogseqFolder` / `matchesIgnoreFilters` in core (`packages/core/src/ignore.ts`): the Obsidian datasource filters `listPages`/`searchPages` by Obsidian's native `userIgnoreFilters` (which Dataview ignores) **and** always drops the `logseq/` folder — whose `bak/`+`.recycle/` markdown backups Dataview was indexing as phantom thoughts that injected phantom parent/child links into the real pages; the Logseq datasource drops the same folder defensively when a file path is resolvable.
- **Mobile mode** — a distinct touch UI behind a per-editor `mobile` flag + a "Mobile mode (testing)" toggle (Obsidian auto-detects via `Platform.isMobile`; Logseq has no mobile plugin runtime, so the toggle is the only signal).
    - Handles inert — tap a connector to unlink; create via mobile-only toolbar buttons; title + back/forward dropped (breadcrumb is the nav); taller touch cards.
    - Activating a card recenters without switching the editor page (keeps the drawer open); the active card opens in the editor.
